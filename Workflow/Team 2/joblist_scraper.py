...
# Generate a unique filename per run
timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
output_file = f"joblist_output_{timestamp}.csv"
...
def scrape_page(driver, job_dict):
    try:
        ...
        save_progress(job_dict, output_file)  # Save after scraping this page
...
def next_page(driver):
    try:
        next_page_button = WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.CLASS_NAME, "css-1vk6oae"))
        )
...
def main():
    ...
    url_list = [
        "https://www.joblist.com/search?l=Chicago%2C+IL&q=software+developer&lr=WITHIN_25_MILES",
